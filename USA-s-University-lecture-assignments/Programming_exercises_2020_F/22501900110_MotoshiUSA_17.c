...
int main(void)
{
    ...
    FILE *fp, *fp_csv;
    if ((fp = fopen("22501900110_MotoshiUSA_17.txt", "w")) == NULL)
    {
        printf("Cannot open the file\n");
        exit(1);
    }
    if ((fp_csv = fopen("22501900110_MotoshiUSA_17.csv", "w")) == NULL)
    {
    ...
    fprintf(fp, "%d\n", n);
    n++;
    double b[n], a[n * n], A[4];

    printf("xの値を入力してください。\n");
    for (i = 0; i < 10; i++) /*与えられた数値の入力*/
    {
        scanf("%lf",&x[i]);
    }
    printf("yの値を入力してください。\n");
    ...
    printf("与えられた座標は以下になります\n");
    for (i = 0; i < 10; i++) /*与えられた数値の確認*/
    {
        printf("%lf, ",x[i]);
    }
    printf("\n");
